...
type Card struct {
	...
	Number      string    `json:"-"`    // Зашифрованное значение
	ExpiryMonth int       `json:"-"`    // Зашифрованное значение
	ExpiryYear  int       `json:"-"`    // Зашифрованное значение
	CVVHash     string    `json:"-"`    // Хеш CVV
	HMAC        string    `json:"hmac"` // Для проверки целостности
	CreatedAt   time.Time `json:"created_at"`
}
