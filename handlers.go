...
func GetUserAccountsHandler(w http.ResponseWriter, r *http.Request) {
	...
	accounts := GetUserAccounts(userID)
	logrus.WithField("user_id", userID).Info("Fetched user accounts")
...
func GenerateCardHandler(w http.ResponseWriter, r *http.Request) {
	...
	if account.UserID != authUserID {
		respondError(w, http.StatusForbidden, "Access denied")
		return
	}

	card, err := GenerateEncryptedCard(req.AccountID)
	if err != nil {
		respondError(w, http.StatusInternalServerError, "Failed to generate card")
		return
	}

	logrus.WithField("account_id", req.AccountID).Info("Card generated")
...
func PredictBalanceHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	accountID := vars["accountId"]
	days, err := time.ParseDuration(r.URL.Query().Get("days") + "h")
	if err != nil || days.Hours() > 365*24 {
		respondError(w, http.StatusBadRequest, "Invalid days parameter")
		return
	}
	...
	prediction := PredictAccountBalance(accountID, int(days.Hours()/24))
...
func (c *Card) ToResponse() CardResponse {
	return CardResponse{
		...
		LastFour:   "****" + c.Number[len(c.Number)-4:],
		ExpiryDate: fmt.Sprintf("%02d/%d", c.ExpiryMonth, c.ExpiryYear),
