...
func PredictAccountBalance(accountID string, days int) map[string]interface{} {
	transactions := GetAccountTransactions(accountID)
	predictedBalance, _ := GetAccount(accountID)

	balance := predictedBalance.Balance
	predictions := []decimal.Decimal{balance}

	for i := 1; i <= days; i++ {
		dailyChange := decimal.Zero
		for _, tx := range transactions {
			if tx.Timestamp.After(time.Now()) && tx.Timestamp.Before(time.Now().AddDate(0, 0, i)) {
				if tx.ToAccountID == accountID {
					dailyChange = dailyChange.Add(tx.Amount)
				} else {
					dailyChange = dailyChange.Sub(tx.Amount)
				}
			}
		}
		balance = balance.Add(dailyChange)
		predictions = append(predictions, balance)
	}

	return map[string]interface{}{
		"account_id":    accountID,
		"start_balance": predictions[0],
		"end_balance":   predictions[len(predictions)-1],
